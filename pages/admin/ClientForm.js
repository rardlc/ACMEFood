...
function ClientForm({ formName, ClientId, addFormToParent, accountId }) {
        ...
        if (addrs !== dbAddrs) {
          ...
          axios(postConfig3).then(
            res => {
              console.log(res)
              location.reload()
              //save where we are and refresh the page
            }
          )
        }
  ...
  function clientAdd(event) {
    ...
    restChanges["oldArr"].forEach(
      dbRest => {
        //////console.log(ogRest)
        if (!(restArr.includes(dbRest)) && !(restChanges["remove"].includes(dbRest))) {
          restChanges.remove.push(dbRest)
    ...
    restArr.forEach(
      currRest => {
        if (!(restChanges["oldArr"].includes(currRest)) && !(restChanges["add"].includes(currRest))) {
    ...
    if (clientId === "" || clientId <= 0) {
      ...
      axios(postConfig).then(
        res => {
          alert(`Saved Client ${currentForm["fname"]}`)
  ...
  function setAddrForm(addrsPkg) {
    ...
    setAddrs({ "primary": addrsPkg["primary"], "addrs": addrsPkg["addrs"], "remove": addrsPkg["remove"] })
      ...
      >
        ...
        <div className="multi-line">
          ...
          >
            ...
            {
              calBool ?
                <Calendar currWeeklyMeals={weeklyMeals} sendDate={setCalDate} formCallback={setCal} cal={cal} addrs={addrs.addrs} clientId={clientId} dietId={form.getFieldValue("diet")} />
                : null
            }
          </Form.Item>
